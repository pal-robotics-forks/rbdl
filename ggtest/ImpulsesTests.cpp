...
class ImpulsesFixture : public ::testing::Test {
protected:
  virtual void SetUp () {
    ClearLogOutput();
    model = new Model;
...
TEST_F(ImpulsesFixture, TestContactImpulse) {
  ...
  // cout << "Point Velocity = " << point_velocity << endl;
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (0., 0., 0.), point_velocity, TEST_PREC));
}
...
TEST_F(ImpulsesFixture, TestContactImpulseRotatedCollisionVelocity) {
  ...
  // cout << "Point Velocity = " << point_velocity << endl;
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 2., 3.), point_velocity, TEST_PREC));
}
...
TEST_F(ImpulsesFixture, TestContactImpulseRangeSpaceSparse) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (qdot_post_direct, qdot_post_rangespace, TEST_PREC));
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 2., 3.), point_velocity_rangespace, TEST_PREC));
}
...
TEST_F(ImpulsesFixture, TestContactImpulseNullSpace) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (qdot_post_direct, qdot_post_nullspace, TEST_PREC));
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 2., 3.), point_velocity_nullspace, TEST_PREC));
}
