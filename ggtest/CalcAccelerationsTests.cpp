...
TEST_F(FixedBase3DoF, TestCalcPointSimple) {
  ...
  point_acceleration = CalcPointAcceleration(*model, *model_data, Q, QDot, QDDot, ref_body_id, point_position);
...
TEST_F(FixedBase3DoF, TestAccelerationLinearFuncOfQddot) {
  ...
  Vector3d acc_1 = CalcPointAcceleration(*model, *model_data, Q, QDot, qddot_1, ref_body_id, point_position);
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR(net_acc, acc_new, TEST_PREC));
}
...
TEST_F(FixedBase3DoF, TestCalcPointRotationFixedJoint) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (point_acceleration_reference, point_acceleration, TEST_PREC));
}
