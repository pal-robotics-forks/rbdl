...
class CustomJointSingleBodyFixture: public ::testing::Test {
protected:
  virtual void SetUp () {
...
TEST_F ( CustomJointSingleBodyFixture, UpdateKinematics ) {
  ...
  for(int idx =0; idx < NUMBER_OF_MODELS; ++idx){
    ...
    EXPECT_TRUE(EIGEN_MATRIX_NEAR (
                  reference_model.at(idx).X_base[reference_body_id.at(idx)].toMatrix(),
                custom_model.at(idx).X_base[custom_body_id.at(idx)].toMatrix(),
        TEST_PREC));
...
TEST_F (CustomJointSingleBodyFixture, ForwardDynamics) {
  ...
  for(int idx =0; idx < NUMBER_OF_MODELS; ++idx){
    ...
    EXPECT_TRUE(EIGEN_MATRIX_NEAR ( qddotRef,
                                    qddotCus,
