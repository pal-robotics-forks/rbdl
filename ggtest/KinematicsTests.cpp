...
protected:
  virtual void SetUp () {
    ...
    body_a_id = model->AddBody(*model_data,*model_data, 0, Xtrans(Vector3d(0., 0., 0.)), joint_a, body_a);

    body_b = Body (1., Vector3d (0., 1., 0.), Vector3d (1., 1., 1.));
    joint_b = Joint ( SpatialVectord (0., 1., 0., 0., 0., 0.));

    body_b_id = model->AddBody(*model_data,*model_data, body_a_id, Xtrans(Vector3d(1., 0., 0.)), joint_b, body_b);

    body_c = Body (1., Vector3d (0., 0., 1.), Vector3d (1., 1., 1.));
    joint_c = Joint ( SpatialVectord (0., 0., 1., 0., 0., 0.));

    body_c_id = model->AddBody(*model_data,*model_data, body_b_id, Xtrans(Vector3d(0., 1., 0.)), joint_c, body_c);

    body_d = Body (1., Vector3d (1., 0., 0.), Vector3d (1., 1., 1.));
    joint_c = Joint ( SpatialVectord (1., 0., 0., 0., 0., 0.));

    body_d_id = model->AddBody(*model_data,*model_data, body_c_id, Xtrans(Vector3d(0., 0., -1.)), joint_c, body_d);
...
TEST_F(KinematicsFixture, TestPositionNeutral) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (0., 0., 0.), CalcBodyToBaseCoordinates(*model, Q, body_a_id, Vector3d (0., 0., 0.), true), TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 0., 0.), CalcBodyToBaseCoordinates(*model, Q, body_b_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 1., 0.), CalcBodyToBaseCoordinates(*model, Q, body_c_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 1., -1.), CalcBodyToBaseCoordinates(*model, Q, body_d_id, Vector3d (0., 0., 0.), true), TEST_PREC) );
}
...
TEST_F(KinematicsFixture, TestPositionBaseRotated90Deg) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (0., 0., 0.), CalcBodyToBaseCoordinates(*model, Q, body_a_id, Vector3d (0., 0., 0.), true), TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (0., 1., 0.), CalcBodyToBaseCoordinates(*model, Q, body_b_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (-1., 1., 0.),CalcBodyToBaseCoordinates(*model, Q, body_c_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (-1., 1., -1.), CalcBodyToBaseCoordinates(*model, Q, body_d_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
}
...
TEST_F(KinematicsFixture, TestPositionBaseRotatedNeg45Deg) {
  ...
  //	cout << LogOutput.str() << endl;
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (0., 0., 0.), CalcBodyToBaseCoordinates(*model, Q, body_a_id, Vector3d (0., 0., 0.), true), TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (0.707106781186547, -0.707106781186547, 0.), CalcBodyToBaseCoordinates(*model, Q, body_b_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (sqrt(2.0), 0., 0.),CalcBodyToBaseCoordinates(*model, Q, body_c_id, Vector3d (0., 0., 0.), true), TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (sqrt(2.0), 0., -1.), CalcBodyToBaseCoordinates(*model, Q, body_d_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
}
...
TEST_F(KinematicsFixture, TestPositionBodyBRotated90Deg) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 0., 0.), CalcBodyToBaseCoordinates(*model, Q, body_b_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 1., 0.),CalcBodyToBaseCoordinates(*model, Q, body_c_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (0., 1., 0.),CalcBodyToBaseCoordinates(*model, Q, body_d_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
}
...
TEST_F(KinematicsFixture, TestPositionBodyBRotatedNeg45Deg) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 0., 0.), CalcBodyToBaseCoordinates(*model, Q, body_b_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1., 1., 0.),CalcBodyToBaseCoordinates(*model, Q, body_c_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (1 + 0.707106781186547, 1., -0.707106781186547), CalcBodyToBaseCoordinates(*model, Q, body_d_id, Vector3d (0., 0., 0.), true),  TEST_PREC) );
}
