...
TEST_F (FloatingBase12DoF, TestSparseFactorizationLTL) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (H, LTL, TEST_PREC));
}
...
TEST_F (FloatingBase12DoF, TestSparseSolveLx) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Q, x, TEST_PREC));
}
...
TEST_F (FixedBase6DoF12DoFFloatingBase, ForwardDynamicsContactsSparse) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (QDDot, QDDot_var1, TEST_PREC));
}

TEST (SparseFactiorizationTests, TestSparseFactorizationMultiDof) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (x_emulated, x_3dof,1.0e-9));
...
TEST (SparseFactiorizationTests, TestSparseFactorizationMultiDofAndFixed) {
  Model model_emulated;
  Model model_3dof;
