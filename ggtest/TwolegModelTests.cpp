...
TEST (TwolegModelTests, TestForwardDynamicsContactsDirectFootmodel ) {
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (Vector3d (0., 0., 0.), contact_accel_left, TEST_PREC));

  delete model;
}

TEST (TwolegModelTests, TestClearContactsInertiaMatrix ) {
  Model* model = new Model;
  ...
  EXPECT_TRUE(EIGEN_MATRIX_NEAR (QDDot_lag, QDDot_aba, TEST_PREC * QDDot_lag.norm()));
