                        ...
                        std::vector<double> &max_effort, bool verbose)
{
  ...
  return URDFReadFromURDF(urdf_model, model, model_data, floatingBaseType, joint_names,
                          position_min, position_max, vel_min, vel_max, damping, friction,
                          max_effort, verbose);
}
                             ...
                             FloatingBaseType floatingBaseType, bool verbose)
{
  ...
  if (!construct_model_depht_first(*model, model_data, urdf_model, floatingBaseType,
                                   tipLinks, verbose))
  {
    cerr << "Error constructing model from urdf file." << endl;
    return false;
