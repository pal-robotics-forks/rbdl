...
struct RBDL_DLLAPI SpatialRigidBodyInertia{
  SpatialRigidBodyInertia<T>() :
    m (0.),
    h (Vector3<T>::Zero(3,1)),
    Ixx (0.), Iyx(0.), Iyy(0.), Izx(0.), Izy(0.), Izz(0.)
  {}
  SpatialRigidBodyInertia<T> (
      T mass, const Vector3<T>  &com_mass, const Matrix3<T> &inertia) :
    m (mass), h (com_mass),
    Ixx (inertia(0,0)),
  ...
  static SpatialRigidBodyInertia<T> createFromMassComInertiaC (T mass, const Vector3<T>  &com, const Matrix3<T> &inertia_C) {
...
struct RBDL_DLLAPI SpatialTransform {
  SpatialTransform() :
    E (Matrix3<T>::Identity(3,3)),
    r (Vector3<T>::Zero(3,1))
  {}
  ...
  */
  ...
  SpatialVector<T> applyAdjoint (const SpatialVector<T> &f_sp) {
    Vector3<T>  En_rxf = E * (Vector3<T>  (f_sp[0], f_sp[1], f_sp[2]) - r.cross(Vector3<T>  (f_sp[3], f_sp[4], f_sp[5])));
    //		Vector3<T>  En_rxf = E * (Vector3<T>  (f_sp[0], f_sp[1], f_sp[2]) - r.cross(Eigen::Map<Vector3> (&(f_sp[3]))));
